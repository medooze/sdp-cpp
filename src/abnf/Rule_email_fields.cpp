...
Rule* Rule_email_fields::clone() const
{
  return new Rule_email_fields(this->spelling, this->rules);
}

void* Rule_email_fields::accept(Visitor& visitor)
{
  return visitor.visit(this);
}

Rule_email_fields* Rule_email_fields::parse(ParserContext& context)
{
      ...
      while (f1)
      {
        ...
        parsed = false;
        {
          ...
          if (parsed)
          {
            bool f2 = true;
            int c2 = 0;
            Rule* rule = Rule_email_field::parse(context);
            if ((f2 = rule != NULL))
            {
  ...
  if (parsed)
  {
    rule = new Rule_email_fields(context.text.substr(a0.start, a0.end - a0.start), a0.rules);
  }
  else
  {
    context.index = s0;
