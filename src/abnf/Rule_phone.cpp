...
Rule_phone::Rule_phone(const Rule_phone& rule) : Rule(rule)
{
}

Rule_phone& Rule_phone::operator=(const Rule_phone& rule)
{
  Rule::operator=(rule);
  return *this;
}
...
const Rule_phone* Rule_phone::parse(ParserContext& context)
{
  ...
  rule = NULL;
  if (parsed)
    rule = new Rule_phone(context.text.substr(s0, context.index - s0), e0);
  else
    context.index = s0;

  context.pop("phone", parsed);

  return (Rule_phone*)rule;
}
