...
Rule_scheme::Rule_scheme(const Rule_scheme& rule) : Rule(rule)
{
}

Rule_scheme& Rule_scheme::operator=(const Rule_scheme& rule)
{
  Rule::operator=(rule);
  return *this;
}
...
const Rule_scheme* Rule_scheme::parse(ParserContext& context)
{
  ...
  rule = NULL;
  if (parsed)
    rule = new Rule_scheme(context.text.substr(s0, context.index - s0), e0);
  else
    context.index = s0;
