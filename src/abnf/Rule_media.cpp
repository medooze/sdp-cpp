...
const Rule_media* Rule_media::clone() const
{
  return new Rule_media(this->spelling, this->rules);
}

void* Rule_media::accept(Visitor& visitor) const
{
  return visitor.visit(this);
}

const Rule_media* Rule_media::parse(ParserContext& context)
{
  context.push("media");
  ...
  rule = NULL;
  if (parsed)
    rule = new Rule_media(context.text.substr(s0, context.index - s0), e0);
  else
    context.index = s0;
