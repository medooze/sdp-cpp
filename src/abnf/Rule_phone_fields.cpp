...
const Rule_phone_fields* Rule_phone_fields::clone() const
{
  return new Rule_phone_fields(this->spelling, this->rules);
}

void* Rule_phone_fields::accept(Visitor& visitor) const
{
  return visitor.visit(this);
}

const Rule_phone_fields* Rule_phone_fields::parse(ParserContext& context)
{
          ...
          {
            {
              ...
              if (parsed)
              {
                ...
                {
                  rule = Rule_phone_field::parse(context);
                  if ((f2 = rule != NULL))
                  {
                    e2.push_back(rule);
                    c2++;
  ...
  rule = NULL;
  if (parsed)
    rule = new Rule_phone_fields(context.text.substr(s0, context.index - s0), e0);
  else
    context.index = s0;
