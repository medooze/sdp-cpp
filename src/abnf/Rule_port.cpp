...
const Rule_port* Rule_port::clone() const
{
  return new Rule_port(this->spelling, this->rules);
}

void* Rule_port::accept(Visitor& visitor) const
{
  return visitor.visit(this);
}

const Rule_port* Rule_port::parse(ParserContext& context)
{
  context.push("port");
  ...
  rule = NULL;
  if (parsed)
    rule = new Rule_port(context.text.substr(s0, context.index - s0), e0);
  else
    context.index = s0;
