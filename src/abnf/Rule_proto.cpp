...
Rule* Rule_proto::clone() const
{
  return new Rule_proto(this->spelling, this->rules);
}

void* Rule_proto::accept(Visitor& visitor)
{
  return visitor.visit(this);
}

Rule_proto* Rule_proto::parse(ParserContext& context)
{
  context.push("proto");
  ...
  if (parsed)
  {
    rule = new Rule_proto(context.text.substr(a0.start, a0.end - a0.start), a0.rules);
  }
  else
  {
    context.index = s0;
