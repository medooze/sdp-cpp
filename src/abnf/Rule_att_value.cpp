...
const Rule_att_value* Rule_att_value::clone() const
{
  return new Rule_att_value(this->spelling, this->rules);
}

void* Rule_att_value::accept(Visitor& visitor) const
{
  return visitor.visit(this);
}

const Rule_att_value* Rule_att_value::parse(ParserContext& context)
{
  ...
  rule = NULL;
  if (parsed)
    rule = new Rule_att_value(context.text.substr(s0, context.index - s0), e0);
  else
    context.index = s0;
