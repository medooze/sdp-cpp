...
const Rule_bwtype* Rule_bwtype::clone() const
{
  return new Rule_bwtype(this->spelling, this->rules);
}

void* Rule_bwtype::accept(Visitor& visitor) const
{
  return visitor.visit(this);
}

const Rule_bwtype* Rule_bwtype::parse(ParserContext& context)
{
  context.push("bwtype");
  ...
  rule = NULL;
  if (parsed)
    rule = new Rule_bwtype(context.text.substr(s0, context.index - s0), e0);
  else
    context.index = s0;
