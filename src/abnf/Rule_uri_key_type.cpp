...
Rule* Rule_uri_key_type::clone() const
{
  return new Rule_uri_key_type(this->spelling, this->rules);
}

void* Rule_uri_key_type::accept(Visitor& visitor)
{
  return visitor.visit(this);
}

Rule_uri_key_type* Rule_uri_key_type::parse(ParserContext& context)
{
  ...
  parsed = false;
  {
    ...
    if (parsed)
    {
      bool f1 = true;
      int c1 = 0;
      Rule* rule = Terminal_StringValue::parse(context, "uri");
      if ((f1 = rule != NULL))
      {
  ...
  if (parsed)
  {
    rule = new Rule_uri_key_type(context.text.substr(a0.start, a0.end - a0.start), a0.rules);
  }
  else
  {
    context.index = s0;
