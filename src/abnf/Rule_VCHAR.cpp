...
const Rule_VCHAR* Rule_VCHAR::clone() const
{
  return new Rule_VCHAR(this->spelling, this->rules);
}

void* Rule_VCHAR::accept(Visitor& visitor) const
{
  return visitor.visit(this);
}

const Rule_VCHAR* Rule_VCHAR::parse(ParserContext& context)
{
  context.push("VCHAR");
  ...
  rule = NULL;
  if (parsed)
    rule = new Rule_VCHAR(context.text.substr(s0, context.index - s0), e0);
  else
    context.index = s0;
