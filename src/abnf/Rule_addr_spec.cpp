...
Rule* Rule_addr_spec::clone() const
{
  return new Rule_addr_spec(this->spelling, this->rules);
}

void* Rule_addr_spec::accept(Visitor& visitor)
{
  return visitor.visit(this);
}

Rule_addr_spec* Rule_addr_spec::parse(ParserContext& context)
{
  ...
  parsed = false;
  {
    ...
    if (parsed)
    {
      bool f1 = true;
      int c1 = 0;
      Rule* rule = Rule_local_part::parse(context);
      if ((f1 = rule != NULL))
      {
    ...
    if (parsed)
    {
      bool f1 = true;
      int c1 = 0;
      Rule* rule = Rule_domain::parse(context);
      if ((f1 = rule != NULL))
      {
        a1.add(rule, context.index);
        c1++;
  ...
  if (parsed)
  {
    rule = new Rule_addr_spec(context.text.substr(a0.start, a0.end - a0.start), a0.rules);
  }
  else
  {
    context.index = s0;
