...
Rule_media_description::Rule_media_description(const Rule_media_description& rule) : Rule(rule)
{
}

Rule_media_description& Rule_media_description::operator=(const Rule_media_description& rule)
{
  Rule::operator=(rule);
  return *this;
}
...
const Rule_media_description* Rule_media_description::parse(ParserContext& context)
{
  ...
  {
    {
      ...
      if (parsed)
      {
        ...
        {
          rule = Rule_media_field::parse(context);
          if ((f1 = rule != NULL))
          {
            e1.push_back(rule);
            c1++;
  ...
  rule = NULL;
  if (parsed)
    rule = new Rule_media_description(context.text.substr(s0, context.index - s0), e0);
  else
    context.index = s0;

  context.pop("media-description", parsed);

  return (Rule_media_description*)rule;
}
