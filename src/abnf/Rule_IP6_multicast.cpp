...
const Rule_IP6_multicast* Rule_IP6_multicast::clone() const
{
  return new Rule_IP6_multicast(this->spelling, this->rules);
}

void* Rule_IP6_multicast::accept(Visitor& visitor) const
{
  return visitor.visit(this);
}

const Rule_IP6_multicast* Rule_IP6_multicast::parse(ParserContext& context)
{
  ...
  rule = NULL;
  if (parsed)
    rule = new Rule_IP6_multicast(context.text.substr(s0, context.index - s0), e0);
  else
    context.index = s0;
