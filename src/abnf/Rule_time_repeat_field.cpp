...
const Rule_time_repeat_field* Rule_time_repeat_field::clone() const
{
  return new Rule_time_repeat_field(this->spelling, this->rules);
}

void* Rule_time_repeat_field::accept(Visitor& visitor) const
{
  return visitor.visit(this);
}

const Rule_time_repeat_field* Rule_time_repeat_field::parse(ParserContext& context)
{
  ...
  {
    {
      ...
      if (parsed)
      {
        ...
        {
          rule = Rule_time_field::parse(context);
          if ((f1 = rule != NULL))
          {
            e1.push_back(rule);
            c1++;
          ...
          {
            {
              ...
              if (parsed)
              {
                ...
                {
                  rule = Rule_repeat_field::parse(context);
                  if ((f2 = rule != NULL))
                  {
                    e2.push_back(rule);
                    c2++;
  ...
  rule = NULL;
  if (parsed)
    rule = new Rule_time_repeat_field(context.text.substr(s0, context.index - s0), e0);
  else
    context.index = s0;
