...
Rule* Rule_segment_nz::clone() const
{
  return new Rule_segment_nz(this->spelling, this->rules);
}

void* Rule_segment_nz::accept(Visitor& visitor)
{
  return visitor.visit(this);
}

Rule_segment_nz* Rule_segment_nz::parse(ParserContext& context)
{
  ...
  if (parsed)
  {
    rule = new Rule_segment_nz(context.text.substr(a0.start, a0.end - a0.start), a0.rules);
  }
  else
  {
    context.index = s0;
