...
const Rule_qcontent* Rule_qcontent::clone() const
{
  return new Rule_qcontent(this->spelling, this->rules);
}

void* Rule_qcontent::accept(Visitor& visitor) const
{
  return visitor.visit(this);
}

const Rule_qcontent* Rule_qcontent::parse(ParserContext& context)
{
  ...
  rule = NULL;
  if (parsed)
    rule = new Rule_qcontent(context.text.substr(s0, context.index - s0), e0);
  else
    context.index = s0;
