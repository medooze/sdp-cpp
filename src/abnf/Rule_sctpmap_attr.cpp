...
const Rule_sctpmap_attr* Rule_sctpmap_attr::clone() const
{
  return new Rule_sctpmap_attr(this->spelling, this->rules);
}

void* Rule_sctpmap_attr::accept(Visitor& visitor) const
{
  return visitor.visit(this);
}

const Rule_sctpmap_attr* Rule_sctpmap_attr::parse(ParserContext& context)
{
  ...
  {
    {
      ...
      if (parsed)
      {
        ...
        {
          rule = Terminal_StringValue::parse(context, "sctpmap:");
          if ((f1 = rule != NULL))
          {
            e1.push_back(rule);
            c1++;
      ...
      if (parsed)
      {
        ...
        for (int i1 = 0; i1 < 1 && f1; i1++)
        {
          rule = Rule_sctpmap_number::parse(context);
          if ((f1 = rule != NULL))
          {
            e1.push_back(rule);
            c1++;
          ...
          {
            {
              ...
              if (parsed)
              {
                ...
                {
                  rule = Rule_streams::parse(context);
                  if ((f2 = rule != NULL))
                  {
                    e2.push_back(rule);
                    c2++;
  ...
  rule = NULL;
  if (parsed)
    rule = new Rule_sctpmap_attr(context.text.substr(s0, context.index - s0), e0);
  else
    context.index = s0;
