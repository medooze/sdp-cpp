...
const Rule_ctext* Rule_ctext::parse(ParserContext& context)
{
  ...
  {
    {
      ...
      if (parsed)
      {
        ...
        for (int i1 = 0; i1 < 1 && f1; i1++)
        {
          rule = Terminal_NumericValue::parse(context, "%d33-39", "[\\x21-\\x27]", 1);
          if ((f1 = rule != NULL))
          {
            e1.push_back(rule);
            c1++;
  ...
  {
    {
      ...
      if (parsed)
      {
        ...
        for (int i1 = 0; i1 < 1 && f1; i1++)
        {
          rule = Terminal_NumericValue::parse(context, "%d42-91", "[\\x2a-\\x5b]", 1);
          if ((f1 = rule != NULL))
          {
            e1.push_back(rule);
            c1++;
  ...
  rule = NULL;
  if (parsed)
    rule = new Rule_ctext(context.text.substr(s0, context.index - s0), e0);
  else
    context.index = s0;
