...
Rule_ctext::Rule_ctext(const Rule_ctext& rule) : Rule(rule)
{
}

Rule_ctext& Rule_ctext::operator=(const Rule_ctext& rule)
{
  Rule::operator=(rule);
  return *this;
}
...
const Rule_ctext* Rule_ctext::parse(ParserContext& context)
{
  ...
  }
  {
    ...
    if (parsed)
    {
      ...
      for (int i1 = 0; i1 < 1 && f1; i1++)
      {
        const Rule* rule = Terminal_NumericValue::parse(context, "%d33-39", "[\\x21-\\x27]", 1);
        if ((f1 = rule != NULL))
        {
          a1.add(*rule, context.index);
          c1++;
  ...
  }
  {
    ...
    if (parsed)
    {
      ...
      for (int i1 = 0; i1 < 1 && f1; i1++)
      {
        const Rule* rule = Terminal_NumericValue::parse(context, "%d42-91", "[\\x2a-\\x5b]", 1);
        if ((f1 = rule != NULL))
        {
          a1.add(*rule, context.index);
          c1++;
  ...
  }
  {
    ...
    if (parsed)
    {
      ...
      for (int i1 = 0; i1 < 1 && f1; i1++)
      {
        const Rule* rule = Terminal_NumericValue::parse(context, "%d93-126", "[\\x5d-\\x7e]", 1);
        if ((f1 = rule != NULL))
        {
          a1.add(*rule, context.index);
          c1++;
