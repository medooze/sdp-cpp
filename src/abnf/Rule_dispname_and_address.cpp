...
const Rule_dispname_and_address* Rule_dispname_and_address::clone() const
{
  return new Rule_dispname_and_address(this->spelling, this->rules);
}

void* Rule_dispname_and_address::accept(Visitor& visitor) const
{
  return visitor.visit(this);
}

const Rule_dispname_and_address* Rule_dispname_and_address::parse(ParserContext& context)
{
  ...
  {
    {
      ...
      if (parsed)
      {
        ...
        for (int i1 = 0; i1 < 1 && f1; i1++)
        {
          rule = Rule_addr_spec::parse(context);
          if ((f1 = rule != NULL))
          {
            e1.push_back(rule);
            c1++;
  ...
  rule = NULL;
  if (parsed)
    rule = new Rule_dispname_and_address(context.text.substr(s0, context.index - s0), e0);
  else
    context.index = s0;
