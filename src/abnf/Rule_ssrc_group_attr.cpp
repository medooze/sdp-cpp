...
const Rule_ssrc_group_attr* Rule_ssrc_group_attr::clone() const
{
  return new Rule_ssrc_group_attr(this->spelling, this->rules);
}

void* Rule_ssrc_group_attr::accept(Visitor& visitor) const
{
  return visitor.visit(this);
}

const Rule_ssrc_group_attr* Rule_ssrc_group_attr::parse(ParserContext& context)
{
  ...
  {
    {
      ...
      if (parsed)
      {
        ...
        {
          rule = Terminal_StringValue::parse(context, "ssrc-group:");
          if ((f1 = rule != NULL))
          {
            e1.push_back(rule);
            c1++;
      ...
      if (parsed)
      {
        ...
        for (int i1 = 0; i1 < 1 && f1; i1++)
        {
          rule = Rule_ssrc_group_semantics::parse(context);
          if ((f1 = rule != NULL))
          {
            e1.push_back(rule);
            c1++;
  ...
  rule = NULL;
  if (parsed)
    rule = new Rule_ssrc_group_attr(context.text.substr(s0, context.index - s0), e0);
  else
    context.index = s0;
