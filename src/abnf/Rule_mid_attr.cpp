...
Rule* Rule_mid_attr::clone() const
{
  return new Rule_mid_attr(this->spelling, this->rules);
}

void* Rule_mid_attr::accept(Visitor& visitor)
{
  return visitor.visit(this);
}

Rule_mid_attr* Rule_mid_attr::parse(ParserContext& context)
{
  ...
  parsed = false;
  {
    ...
    if (parsed)
    {
      bool f1 = true;
      int c1 = 0;
      Rule* rule = Terminal_StringValue::parse(context, "mid:");
      if ((f1 = rule != NULL))
      {
  ...
  if (parsed)
  {
    rule = new Rule_mid_attr(context.text.substr(a0.start, a0.end - a0.start), a0.rules);
  }
  else
  {
    context.index = s0;
