...
const Rule_address_and_comment* Rule_address_and_comment::clone() const
{
  return new Rule_address_and_comment(this->spelling, this->rules);
}

void* Rule_address_and_comment::accept(Visitor& visitor) const
{
  return visitor.visit(this);
}

const Rule_address_and_comment* Rule_address_and_comment::parse(ParserContext& context)
{
  ...
  rule = NULL;
  if (parsed)
    rule = new Rule_address_and_comment(context.text.substr(s0, context.index - s0), e0);
  else
    context.index = s0;
