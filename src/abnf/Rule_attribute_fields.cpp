...
Rule_attribute_fields::Rule_attribute_fields(const Rule_attribute_fields& rule) : Rule(rule)
{
}

Rule_attribute_fields& Rule_attribute_fields::operator=(const Rule_attribute_fields& rule)
{
  Rule::operator=(rule);
  return *this;
}
...
const Rule_attribute_fields* Rule_attribute_fields::parse(ParserContext& context)
{
          ...
          {
            {
              ...
              if (parsed)
              {
                ...
                {
                  rule = Rule_attribute_field::parse(context);
                  if ((f2 = rule != NULL))
                  {
                    e2.push_back(rule);
                    c2++;
  ...
  rule = NULL;
  if (parsed)
    rule = new Rule_attribute_fields(context.text.substr(s0, context.index - s0), e0);
  else
    context.index = s0;

  context.pop("attribute-fields", parsed);

  return (Rule_attribute_fields*)rule;
}
