...
const Rule_ssrc_id* Rule_ssrc_id::clone() const
{
  return new Rule_ssrc_id(this->spelling, this->rules);
}

void* Rule_ssrc_id::accept(Visitor& visitor) const
{
  return visitor.visit(this);
}

const Rule_ssrc_id* Rule_ssrc_id::parse(ParserContext& context)
{
  ...
  rule = NULL;
  if (parsed)
    rule = new Rule_ssrc_id(context.text.substr(s0, context.index - s0), e0);
  else
    context.index = s0;
