...
Rule* Rule_path_abempty::clone() const
{
  return new Rule_path_abempty(this->spelling, this->rules);
}

void* Rule_path_abempty::accept(Visitor& visitor)
{
  return visitor.visit(this);
}

Rule_path_abempty* Rule_path_abempty::parse(ParserContext& context)
{
  ...
  if (parsed)
  {
    rule = new Rule_path_abempty(context.text.substr(a0.start, a0.end - a0.start), a0.rules);
  }
  else
  {
    context.index = s0;
