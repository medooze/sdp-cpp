...
Rule* Rule_domain_literal::clone() const
{
  return new Rule_domain_literal(this->spelling, this->rules);
}

void* Rule_domain_literal::accept(Visitor& visitor)
{
  return visitor.visit(this);
}

Rule_domain_literal* Rule_domain_literal::parse(ParserContext& context)
{
      ...
      for (int i1 = 0; i1 < 1 && f1; i1++)
      {
        ...
        parsed = false;
        {
          ...
          if (parsed)
          {
            bool f2 = true;
            int c2 = 0;
            Rule* rule = Rule_CFWS::parse(context);
            if ((f2 = rule != NULL))
            {
      ...
      while (f1)
      {
        ...
        parsed = false;
        {
          ...
          if (parsed)
          {
            bool f2 = true;
            int c2 = 0;
            Rule* rule = Rule_dcontent::parse(context);
            if ((f2 = rule != NULL))
            {
              a2.add(rule, context.index);
              c2++;
