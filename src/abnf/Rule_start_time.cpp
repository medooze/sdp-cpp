...
Rule_start_time::Rule_start_time(const Rule_start_time& rule) : Rule(rule)
{
}

Rule_start_time& Rule_start_time::operator=(const Rule_start_time& rule)
{
  Rule::operator=(rule);
  return *this;
}
...
const Rule_start_time* Rule_start_time::parse(ParserContext& context)
{
  ...
  rule = NULL;
  if (parsed)
    rule = new Rule_start_time(context.text.substr(s0, context.index - s0), e0);
  else
    context.index = s0;

  context.pop("start-time", parsed);

  return (Rule_start_time*)rule;
}
