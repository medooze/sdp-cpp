...
const Rule_component_id* Rule_component_id::clone() const
{
  return new Rule_component_id(this->spelling, this->rules);
}

void* Rule_component_id::accept(Visitor& visitor) const
{
  return visitor.visit(this);
}

const Rule_component_id* Rule_component_id::parse(ParserContext& context)
{
  ...
  rule = NULL;
  if (parsed)
    rule = new Rule_component_id(context.text.substr(s0, context.index - s0), e0);
  else
    context.index = s0;
