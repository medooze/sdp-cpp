...
const Rule_ssrc_group_semantics* Rule_ssrc_group_semantics::clone() const
{
  return new Rule_ssrc_group_semantics(this->spelling, this->rules);
}

void* Rule_ssrc_group_semantics::accept(Visitor& visitor) const
{
  return visitor.visit(this);
}

const Rule_ssrc_group_semantics* Rule_ssrc_group_semantics::parse(ParserContext& context)
{
  ...
  rule = NULL;
  if (parsed)
    rule = new Rule_ssrc_group_semantics(context.text.substr(s0, context.index - s0), e0);
  else
    context.index = s0;
