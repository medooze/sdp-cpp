...
Rule_reg_name::Rule_reg_name(const Rule_reg_name& rule) : Rule(rule)
{
}

Rule_reg_name& Rule_reg_name::operator=(const Rule_reg_name& rule)
{
  Rule::operator=(rule);
  return *this;
}
...
const Rule_reg_name* Rule_reg_name::parse(ParserContext& context)
{
  ...
  rule = NULL;
  if (parsed)
    rule = new Rule_reg_name(context.text.substr(s0, context.index - s0), e0);
  else
    context.index = s0;
