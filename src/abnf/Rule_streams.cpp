...
const Rule_streams* Rule_streams::clone() const
{
  return new Rule_streams(this->spelling, this->rules);
}

void* Rule_streams::accept(Visitor& visitor) const
{
  return visitor.visit(this);
}

const Rule_streams* Rule_streams::parse(ParserContext& context)
{
  ...
  rule = NULL;
  if (parsed)
    rule = new Rule_streams(context.text.substr(s0, context.index - s0), e0);
  else
    context.index = s0;
