...
const Rule_rel_port* Rule_rel_port::clone() const
{
  return new Rule_rel_port(this->spelling, this->rules);
}

void* Rule_rel_port::accept(Visitor& visitor) const
{
  return visitor.visit(this);
}

const Rule_rel_port* Rule_rel_port::parse(ParserContext& context)
{
  ...
  rule = NULL;
  if (parsed)
    rule = new Rule_rel_port(context.text.substr(s0, context.index - s0), e0);
  else
    context.index = s0;
