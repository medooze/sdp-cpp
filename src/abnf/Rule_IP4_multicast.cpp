...
Rule_IP4_multicast::Rule_IP4_multicast(const Rule_IP4_multicast& rule) : Rule(rule)
{
}

Rule_IP4_multicast& Rule_IP4_multicast::operator=(const Rule_IP4_multicast& rule)
{
  Rule::operator=(rule);
  return *this;
}
...
const Rule_IP4_multicast* Rule_IP4_multicast::parse(ParserContext& context)
{
          ...
          {
            {
              ...
              if (parsed)
              {
                ...
                for (int i2 = 0; i2 < 1 && f2; i2++)
                {
                  rule = Rule_decimal_uchar::parse(context);
                  if ((f2 = rule != NULL))
                  {
                    e2.push_back(rule);
                    c2++;
  ...
  rule = NULL;
  if (parsed)
    rule = new Rule_IP4_multicast(context.text.substr(s0, context.index - s0), e0);
  else
    context.index = s0;

  context.pop("IP4-multicast", parsed);

  return (Rule_IP4_multicast*)rule;
}
