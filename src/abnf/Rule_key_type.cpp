...
Rule* Rule_key_type::clone() const
{
  return new Rule_key_type(this->spelling, this->rules);
}

void* Rule_key_type::accept(Visitor& visitor)
{
  return visitor.visit(this);
}

Rule_key_type* Rule_key_type::parse(ParserContext& context)
{
  ...
  parsed = false;
  {
    ...
    if (parsed)
    {
      bool f1 = true;
      int c1 = 0;
      Rule* rule = Rule_prompt_key_type::parse(context);
      if ((f1 = rule != NULL))
      {
  ...
  }
  {
    ...
    if (parsed)
    {
      bool f1 = true;
      int c1 = 0;
      Rule* rule = Rule_clear_key_type::parse(context);
      if ((f1 = rule != NULL))
      {
        a1.add(rule, context.index);
        c1++;
  ...
  }
  {
    ...
    if (parsed)
    {
      bool f1 = true;
      int c1 = 0;
      Rule* rule = Rule_base64_key_type::parse(context);
      if ((f1 = rule != NULL))
      {
        a1.add(rule, context.index);
        c1++;
  ...
  }
  {
    ...
    if (parsed)
    {
      bool f1 = true;
      int c1 = 0;
      Rule* rule = Rule_uri_key_type::parse(context);
      if ((f1 = rule != NULL))
      {
        a1.add(rule, context.index);
        c1++;
  ...
  if (parsed)
  {
    rule = new Rule_key_type(context.text.substr(a0.start, a0.end - a0.start), a0.rules);
  }
  else
  {
    context.index = s0;
