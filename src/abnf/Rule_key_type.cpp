...
const Rule_key_type* Rule_key_type::parse(ParserContext& context)
{
  ...
  parsed = false;
  {
    ...
    if (parsed)
    {
      ...
      {
        const Rule* rule = Rule_prompt_key_type::parse(context);
        if ((f1 = rule != NULL))
        {
  ...
  }
  {
    ...
    if (parsed)
    {
      ...
      for (int i1 = 0; i1 < 1 && f1; i1++)
      {
        const Rule* rule = Rule_clear_key_type::parse(context);
        if ((f1 = rule != NULL))
        {
          a1.add(*rule, context.index);
          c1++;
  ...
  }
  {
    ...
    if (parsed)
    {
      ...
      for (int i1 = 0; i1 < 1 && f1; i1++)
      {
        const Rule* rule = Rule_base64_key_type::parse(context);
        if ((f1 = rule != NULL))
        {
          a1.add(*rule, context.index);
          c1++;
  ...
  }
  {
    ...
    if (parsed)
    {
      ...
      for (int i1 = 0; i1 < 1 && f1; i1++)
      {
        const Rule* rule = Rule_uri_key_type::parse(context);
        if ((f1 = rule != NULL))
        {
          a1.add(*rule, context.index);
          c1++;
