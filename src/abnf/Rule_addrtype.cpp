...
Rule* Rule_addrtype::clone() const
{
  return new Rule_addrtype(this->spelling, this->rules);
}

void* Rule_addrtype::accept(Visitor& visitor)
{
  return visitor.visit(this);
}

Rule_addrtype* Rule_addrtype::parse(ParserContext& context)
{
  ...
  if (parsed)
  {
    rule = new Rule_addrtype(context.text.substr(a0.start, a0.end - a0.start), a0.rules);
  }
  else
  {
    context.index = s0;
