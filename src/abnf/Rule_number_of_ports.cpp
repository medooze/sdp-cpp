...
Rule* Rule_number_of_ports::clone() const
{
  return new Rule_number_of_ports(this->spelling, this->rules);
}

void* Rule_number_of_ports::accept(Visitor& visitor)
{
  return visitor.visit(this);
}

Rule_number_of_ports* Rule_number_of_ports::parse(ParserContext& context)
{
  ...
  if (parsed)
  {
    rule = new Rule_number_of_ports(context.text.substr(a0.start, a0.end - a0.start), a0.rules);
  }
  else
  {
    context.index = s0;
