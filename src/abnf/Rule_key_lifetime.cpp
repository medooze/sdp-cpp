...
Rule_key_lifetime::Rule_key_lifetime(const Rule_key_lifetime& rule) : Rule(rule)
{
}

Rule_key_lifetime& Rule_key_lifetime::operator=(const Rule_key_lifetime& rule)
{
  Rule::operator=(rule);
  return *this;
}
...
const Rule_key_lifetime* Rule_key_lifetime::parse(ParserContext& context)
{
  ...
  rule = NULL;
  if (parsed)
    rule = new Rule_key_lifetime(context.text.substr(s0, context.index - s0), e0);
  else
    context.index = s0;

  context.pop("key-lifetime", parsed);

  return (Rule_key_lifetime*)rule;
}
