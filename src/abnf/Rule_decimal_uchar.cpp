...
const Rule_decimal_uchar* Rule_decimal_uchar::parse(ParserContext& context)
{
  ...
  {
    {
      ...
      if (parsed)
      {
        ...
        {
          rule = Rule_decimal_uchar_25x::parse(context);
          if ((f1 = rule != NULL))
          {
            e1.push_back(rule);
            c1++;
  ...
  {
    {
      ...
      if (parsed)
      {
        ...
        for (int i1 = 0; i1 < 1 && f1; i1++)
        {
          rule = Rule_decimal_uchar_2xx::parse(context);
          if ((f1 = rule != NULL))
          {
            e1.push_back(rule);
            c1++;
  ...
  {
    {
      ...
      if (parsed)
      {
        ...
        for (int i1 = 0; i1 < 1 && f1; i1++)
        {
          rule = Rule_decimal_uchar_1xx::parse(context);
          if ((f1 = rule != NULL))
          {
            e1.push_back(rule);
            c1++;
  ...
  rule = NULL;
  if (parsed)
    rule = new Rule_decimal_uchar(context.text.substr(s0, context.index - s0), e0);
  else
    context.index = s0;
