...
const Rule_time_field* Rule_time_field::clone() const
{
  return new Rule_time_field(this->spelling, this->rules);
}

void* Rule_time_field::accept(Visitor& visitor) const
{
  return visitor.visit(this);
}

const Rule_time_field* Rule_time_field::parse(ParserContext& context)
{
  ...
  {
    {
      ...
      if (parsed)
      {
        ...
        for (int i1 = 0; i1 < 1 && f1; i1++)
        {
          rule = Rule_start_time::parse(context);
          if ((f1 = rule != NULL))
          {
            e1.push_back(rule);
            c1++;
      ...
      if (parsed)
      {
        ...
        for (int i1 = 0; i1 < 1 && f1; i1++)
        {
          rule = Rule_stop_time::parse(context);
          if ((f1 = rule != NULL))
          {
            e1.push_back(rule);
            c1++;
  ...
  rule = NULL;
  if (parsed)
    rule = new Rule_time_field(context.text.substr(s0, context.index - s0), e0);
  else
    context.index = s0;
