...
Rule_userinfo::Rule_userinfo(
  const string& spelling, 
  const vector<const Rule*>& rules) : Rule(spelling, rules)
{
}

Rule_userinfo::Rule_userinfo(const Rule_userinfo& rule) : Rule(rule)
{
}

Rule_userinfo& Rule_userinfo::operator=(const Rule_userinfo& rule)
{
  Rule::operator=(rule);
  return *this;
}

const Rule_userinfo* Rule_userinfo::clone() const
{
  return new Rule_userinfo(this->spelling, this->rules);
}

void* Rule_userinfo::accept(Visitor& visitor) const
{
  return visitor.visit(this);
}

const Rule_userinfo* Rule_userinfo::parse(ParserContext& context)
{
  ...
  if (parsed)
  {
    rule = new Rule_userinfo(context.text.substr(a0.start, a0.end - a0.start), a0.rules);
  }
  else
  {
    context.index = s0;
