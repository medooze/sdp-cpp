...
const Rule_local_part* Rule_local_part::parse(ParserContext& context)
{
  ...
  }
  {
    ...
    if (parsed)
    {
      ...
      for (int i1 = 0; i1 < 1 && f1; i1++)
      {
        const Rule* rule = Rule_quoted_string::parse(context);
        if ((f1 = rule != NULL))
        {
          a1.add(*rule, context.index);
          c1++;
  ...
  }
  {
    ...
    if (parsed)
    {
      ...
      for (int i1 = 0; i1 < 1 && f1; i1++)
      {
        const Rule* rule = Rule_obs_local_part::parse(context);
        if ((f1 = rule != NULL))
        {
          a1.add(*rule, context.index);
          c1++;
  ...
  if (parsed)
  {
    rule = new Rule_local_part(context.text.substr(a0.start, a0.end - a0.start), a0.rules);
  }
  else
  {
    context.index = s0;
