...
const Rule_rel_addr* Rule_rel_addr::clone() const
{
  return new Rule_rel_addr(this->spelling, this->rules);
}

void* Rule_rel_addr::accept(Visitor& visitor) const
{
  return visitor.visit(this);
}

const Rule_rel_addr* Rule_rel_addr::parse(ParserContext& context)
{
  ...
  rule = NULL;
  if (parsed)
    rule = new Rule_rel_addr(context.text.substr(s0, context.index - s0), e0);
  else
    context.index = s0;
