...
Rule* Rule_LF::clone() const
{
  return new Rule_LF(this->spelling, this->rules);
}

void* Rule_LF::accept(Visitor& visitor)
{
  return visitor.visit(this);
}

Rule_LF* Rule_LF::parse(ParserContext& context)
{
  context.push("LF");
  ...
  parsed = false;
  {
    ...
    if (parsed)
    {
      bool f1 = true;
      int c1 = 0;
      Rule* rule = Terminal_NumericValue::parse(context, "%x0A", 0x0A, 0x0A);
      if ((f1 = rule != NULL))
      {
