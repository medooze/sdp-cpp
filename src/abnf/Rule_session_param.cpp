...
const Rule_session_param* Rule_session_param::clone() const
{
  return new Rule_session_param(this->spelling, this->rules);
}

void* Rule_session_param::accept(Visitor& visitor) const
{
  return visitor.visit(this);
}

const Rule_session_param* Rule_session_param::parse(ParserContext& context)
{
  ...
  rule = NULL;
  if (parsed)
    rule = new Rule_session_param(context.text.substr(s0, context.index - s0), e0);
  else
    context.index = s0;
