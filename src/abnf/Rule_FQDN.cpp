...
Rule_FQDN::Rule_FQDN(const Rule_FQDN& rule) : Rule(rule)
{
}

Rule_FQDN& Rule_FQDN::operator=(const Rule_FQDN& rule)
{
  Rule::operator=(rule);
  return *this;
}
...
const Rule_FQDN* Rule_FQDN::parse(ParserContext& context)
{
          ...
          {
            {
              ...
              if (parsed)
              {
                ...
                {
                  rule = Rule_alpha_numeric::parse(context);
                  if ((f2 = rule != NULL))
                  {
                    e2.push_back(rule);
                    c2++;
  ...
  rule = NULL;
  if (parsed)
    rule = new Rule_FQDN(context.text.substr(s0, context.index - s0), e0);
  else
    context.index = s0;

  context.pop("FQDN", parsed);

  return (Rule_FQDN*)rule;
}
