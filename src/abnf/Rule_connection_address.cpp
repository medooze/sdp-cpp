...
Rule_connection_address::Rule_connection_address(const Rule_connection_address& rule) : Rule(rule)
{
}

Rule_connection_address& Rule_connection_address::operator=(const Rule_connection_address& rule)
{
  Rule::operator=(rule);
  return *this;
}
...
const Rule_connection_address* Rule_connection_address::parse(ParserContext& context)
{
  ...
  {
    {
      ...
      if (parsed)
      {
        ...
        {
          rule = Rule_IP4_multicast::parse(context);
          if ((f1 = rule != NULL))
          {
            e1.push_back(rule);
            c1++;
  ...
  {
    {
      ...
      if (parsed)
      {
        ...
        for (int i1 = 0; i1 < 1 && f1; i1++)
        {
          rule = Rule_IP6_multicast::parse(context);
          if ((f1 = rule != NULL))
          {
            e1.push_back(rule);
            c1++;
  ...
  rule = NULL;
  if (parsed)
    rule = new Rule_connection_address(context.text.substr(s0, context.index - s0), e0);
  else
    context.index = s0;

  context.pop("connection-address", parsed);

  return (Rule_connection_address*)rule;
}
