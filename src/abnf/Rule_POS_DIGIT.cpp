...
const Rule_POS_DIGIT* Rule_POS_DIGIT::clone() const
{
  return new Rule_POS_DIGIT(this->spelling, this->rules);
}

void* Rule_POS_DIGIT::accept(Visitor& visitor) const
{
  return visitor.visit(this);
}

const Rule_POS_DIGIT* Rule_POS_DIGIT::parse(ParserContext& context)
{
  ...
  rule = NULL;
  if (parsed)
    rule = new Rule_POS_DIGIT(context.text.substr(s0, context.index - s0), e0);
  else
    context.index = s0;
