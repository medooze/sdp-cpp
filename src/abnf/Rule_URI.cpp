...
Rule* Rule_URI::clone() const
{
  return new Rule_URI(this->spelling, this->rules);
}

void* Rule_URI::accept(Visitor& visitor)
{
  return visitor.visit(this);
}

Rule_URI* Rule_URI::parse(ParserContext& context)
{
  context.push("URI");
  ...
  parsed = false;
  {
    ...
    if (parsed)
    {
      bool f1 = true;
      int c1 = 0;
      Rule* rule = Rule_scheme::parse(context);
      if ((f1 = rule != NULL))
      {
    ...
    if (parsed)
    {
      bool f1 = true;
      int c1 = 0;
      Rule* rule = Rule_hier_part::parse(context);
      if ((f1 = rule != NULL))
      {
        a1.add(rule, context.index);
        c1++;
