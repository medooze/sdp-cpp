...
const Rule_uri_field* Rule_uri_field::clone() const
{
  return new Rule_uri_field(this->spelling, this->rules);
}

void* Rule_uri_field::accept(Visitor& visitor) const
{
  return visitor.visit(this);
}

const Rule_uri_field* Rule_uri_field::parse(ParserContext& context)
{
  ...
  parsed = false;
  {
    ...
    if (parsed)
    {
      ...
      {
        const Rule* rule = Terminal_NumericValue::parse(context, "%x75", 0x75, 1);
        if ((f1 = rule != NULL))
        {
    ...
    if (parsed)
    {
      ...
      for (int i1 = 0; i1 < 1 && f1; i1++)
      {
        const Rule* rule = Rule_URI_reference::parse(context);
        if ((f1 = rule != NULL))
        {
          a1.add(*rule, context.index);
          c1++;
