...
Rule_segment_nz_nc::Rule_segment_nz_nc(const Rule_segment_nz_nc& rule) : Rule(rule)
{
}

Rule_segment_nz_nc& Rule_segment_nz_nc::operator=(const Rule_segment_nz_nc& rule)
{
  Rule::operator=(rule);
  return *this;
}
...
const Rule_segment_nz_nc* Rule_segment_nz_nc::parse(ParserContext& context)
{
  ...
  rule = NULL;
  if (parsed)
    rule = new Rule_segment_nz_nc(context.text.substr(s0, context.index - s0), e0);
  else
    context.index = s0;

  context.pop("segment-nz-nc", parsed);

  return (Rule_segment_nz_nc*)rule;
}
