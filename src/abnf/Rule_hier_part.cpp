...
Rule_hier_part::Rule_hier_part(const Rule_hier_part& rule) : Rule(rule)
{
}

Rule_hier_part& Rule_hier_part::operator=(const Rule_hier_part& rule)
{
  Rule::operator=(rule);
  return *this;
}
...
const Rule_hier_part* Rule_hier_part::parse(ParserContext& context)
{
  ...
  rule = NULL;
  if (parsed)
    rule = new Rule_hier_part(context.text.substr(s0, context.index - s0), e0);
  else
    context.index = s0;
