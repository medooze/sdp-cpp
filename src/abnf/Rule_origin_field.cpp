...
Rule_origin_field::Rule_origin_field(const Rule_origin_field& rule) : Rule(rule)
{
}

Rule_origin_field& Rule_origin_field::operator=(const Rule_origin_field& rule)
{
  Rule::operator=(rule);
  return *this;
}
...
const Rule_origin_field* Rule_origin_field::parse(ParserContext& context)
{
  ...
  {
    {
      ...
      if (parsed)
      {
        ...
        {
          rule = Terminal_NumericValue::parse(context, "%x6f", "[\\x6f]", 1);
          if ((f1 = rule != NULL))
          {
            e1.push_back(rule);
            c1++;
      ...
      if (parsed)
      {
        ...
        for (int i1 = 0; i1 < 1 && f1; i1++)
        {
          rule = Rule_username::parse(context);
          if ((f1 = rule != NULL))
          {
            e1.push_back(rule);
            c1++;
      ...
      if (parsed)
      {
        ...
        for (int i1 = 0; i1 < 1 && f1; i1++)
        {
          rule = Rule_sess_id::parse(context);
          if ((f1 = rule != NULL))
          {
            e1.push_back(rule);
            c1++;
      ...
      if (parsed)
      {
        ...
        for (int i1 = 0; i1 < 1 && f1; i1++)
        {
          rule = Rule_sess_version::parse(context);
          if ((f1 = rule != NULL))
          {
            e1.push_back(rule);
            c1++;
      ...
      if (parsed)
      {
        ...
        for (int i1 = 0; i1 < 1 && f1; i1++)
        {
          rule = Rule_unicast_address::parse(context);
          if ((f1 = rule != NULL))
          {
            e1.push_back(rule);
            c1++;
  ...
  rule = NULL;
  if (parsed)
    rule = new Rule_origin_field(context.text.substr(s0, context.index - s0), e0);
  else
    context.index = s0;

  context.pop("origin-field", parsed);

  return (Rule_origin_field*)rule;
}
