...
const Rule_group_attr* Rule_group_attr::parse(ParserContext& context)
{
  ...
  parsed = false;
  {
    ...
    if (parsed)
    {
      ...
      {
        const Rule* rule = Terminal_StringValue::parse(context, "group:");
        if ((f1 = rule != NULL))
        {
    ...
    if (parsed)
    {
      ...
      for (int i1 = 0; i1 < 1 && f1; i1++)
      {
        const Rule* rule = Rule_group_semantics::parse(context);
        if ((f1 = rule != NULL))
        {
          a1.add(*rule, context.index);
          c1++;
      ...
      while (f1)
      {
        ...
        parsed = false;
        {
          ...
          if (parsed)
          {
            ...
            for (int i2 = 0; i2 < 1 && f2; i2++)
            {
              const Rule* rule = Rule_identification_tag::parse(context);
              if ((f2 = rule != NULL))
              {
                a2.add(*rule, context.index);
                c2++;
