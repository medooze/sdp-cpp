...
Rule* Rule_group_attr::clone() const
{
  return new Rule_group_attr(this->spelling, this->rules);
}

void* Rule_group_attr::accept(Visitor& visitor)
{
  return visitor.visit(this);
}

Rule_group_attr* Rule_group_attr::parse(ParserContext& context)
{
  ...
  parsed = false;
  {
    ...
    if (parsed)
    {
      bool f1 = true;
      int c1 = 0;
      Rule* rule = Terminal_StringValue::parse(context, "group:");
      if ((f1 = rule != NULL))
      {
    ...
    if (parsed)
    {
      bool f1 = true;
      int c1 = 0;
      Rule* rule = Rule_group_semantics::parse(context);
      if ((f1 = rule != NULL))
      {
        a1.add(rule, context.index);
        c1++;
      ...
      while (f1)
      {
        ...
        parsed = false;
        {
          ...
          if (parsed)
          {
            bool f2 = true;
            int c2 = 0;
            Rule* rule = Rule_identification_tag::parse(context);
            if ((f2 = rule != NULL))
            {
              a2.add(rule, context.index);
              c2++;
  ...
  if (parsed)
  {
    rule = new Rule_group_attr(context.text.substr(a0.start, a0.end - a0.start), a0.rules);
  }
  else
  {
    context.index = s0;
