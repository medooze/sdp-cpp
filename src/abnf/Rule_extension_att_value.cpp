...
Rule* Rule_extension_att_value::clone() const
{
  return new Rule_extension_att_value(this->spelling, this->rules);
}

void* Rule_extension_att_value::accept(Visitor& visitor)
{
  return visitor.visit(this);
}

Rule_extension_att_value* Rule_extension_att_value::parse(ParserContext& context)
{
  ...
  if (parsed)
  {
    rule = new Rule_extension_att_value(context.text.substr(a0.start, a0.end - a0.start), a0.rules);
  }
  else
  {
    context.index = s0;
