...
Rule* Rule_WSP::clone() const
{
  return new Rule_WSP(this->spelling, this->rules);
}

void* Rule_WSP::accept(Visitor& visitor)
{
  return visitor.visit(this);
}

Rule_WSP* Rule_WSP::parse(ParserContext& context)
{
  context.push("WSP");
  ...
  if (parsed)
  {
    rule = new Rule_WSP(context.text.substr(a0.start, a0.end - a0.start), a0.rules);
  }
  else
  {
    context.index = s0;
