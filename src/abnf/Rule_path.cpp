...
Rule_path::Rule_path(const Rule_path& rule) : Rule(rule)
{
}

Rule_path& Rule_path::operator=(const Rule_path& rule)
{
  Rule::operator=(rule);
  return *this;
}
...
const Rule_path* Rule_path::parse(ParserContext& context)
{
  ...
  rule = NULL;
  if (parsed)
    rule = new Rule_path(context.text.substr(s0, context.index - s0), e0);
  else
    context.index = s0;
