...
Rule* Rule_integer::clone() const
{
  return new Rule_integer(this->spelling, this->rules);
}

void* Rule_integer::accept(Visitor& visitor)
{
  return visitor.visit(this);
}

Rule_integer* Rule_integer::parse(ParserContext& context)
{
  ...
  if (parsed)
  {
    rule = new Rule_integer(context.text.substr(a0.start, a0.end - a0.start), a0.rules);
  }
  else
  {
    context.index = s0;
